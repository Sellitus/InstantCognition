...
const rendererErrorHandler = require('./utils/rendererErrorHandler');
...
let toggleShortcut = 'CmdOrCtrl+Space';
...
if (config && config.settings) {
  multiCognition = config.settings.multicognition || false;
  openLinkExternal = config.settings.openLinkExternal || false;
  defaultCognitizer = config.settings.defaultCognitizer || 'custom1';
}

// Apply loaded baseUrls if available
if (config && config.baseUrls) {
...
options.forEach((option) => {
  ...
  if (Array.isArray(config.baseUrls)) {
    let newBaseUrls = {};
    config.baseUrls.forEach((item, index) => {
      // Handle both string URLs and object format
      if (typeof item === 'string') {
        // Legacy format - just a URL string
        newBaseUrls[`custom${index + 1}`] = {
          name: `Custom ${index + 1}`,
          url: item
        };
      } else if (item && typeof item === 'object') {
        // Object format with name and url
        newBaseUrls[`custom${index + 1}`] = item;
      }
    });
...
function collapseMenu() {
  ...
  let menuIcon = document.getElementById('toggle-button');
  menuIcon.innerHTML =
    '<img src="./assets/icon.png" class="collapsed-menu-logo-instant-cognition">';
...
function setCollapsedMenuIcons(options) {
  options.forEach((option) => {
    ...
    if (fullName.substring(0, 4) === 'Back') {
      ...
      var img = new Image();
      option.innerHTML =
        '<img src="./assets/ai-logos/copilot-logo-black-and-white.png" alt="Copilot" id="copilot-logo" class="collapsed-menu-logo">';
    } else if (fullName === 'Perplexity') {
      option.innerHTML =
        '<img src="./assets/ai-logos/perplexity-logo-white.png" alt="Perplexity" id="perplexity-logo" class="collapsed-menu-logo">';
    } else if (fullName === 'Gemini') {
      option.innerHTML =
        '<img src="./assets/ai-logos/gemini-logo-black-and-white.png" alt="gemini" id="gemini-logo" class="collapsed-menu-logo">';
    } else if (fullName === 'Claude') {
      option.innerHTML =
        '<img src="./assets/ai-logos/claude-logo-white.png" alt="Claude" id="claude-logo" class="collapsed-menu-logo">';
    } else if (fullName === 'ChatGPT') {
      option.innerHTML =
        '<img src="./assets/ai-logos/chatgpt-logo-white.png" alt="ChatGPT" id="chatgpt-logo" class="collapsed-menu-logo">';
    } else if (fullName === 'Perplexity Labs') {
      option.innerHTML =
        '<img src="./assets/ai-logos/perplexity-labs-logo-white.png" alt="Perplexity Labs" id="perplexity-labs-logo" class="collapsed-menu-logo-long">';
    } else if (fullName === 'Mistral') {
      option.innerHTML =
        '<img src="./assets/ai-logos/mistral-logo-black-and-white.png" alt="mistral" id="mistral-logo" class="collapsed-menu-logo">';
    } else {
...
function closeActiveCognitizer() {
  let activeWebview = getActiveWebview();
  if (activeWebview) {
    selectCognitizer(activeWebview.id);
  }
}

let lastSelectCognitizerTime = 0;
function selectCognitizer(newCognitizerId, toggle_override = false, skip_timeout = false) {
  // Add timestamp check with 25ms cooldown
  const now = Date.now();
  if (skip_timeout === false && now - lastSelectCognitizerTime < 25) {
    return;
  }
  lastSelectCognitizerTime = now;

  if (!toggle_override && !multiCognition && currentCognitizerId === newCognitizerId) return;

  if (newCognitizerId === null) {
    newCognitizerId = currentCognitizerId;
  }

  // Check if the user is deselecting the last selected cognitizer when in multi-cognition mode, if so do nothing. Comment this out to have multicognition mode get disabled instead
  if (
    multiCognition &&
    activeCognitizers[newCognitizerId] &&
    Object.values(activeCognitizers).filter(Boolean).length === 1
  ) {
    return;
  }
  ...
  if (multiCognition && activeCognitizers[newCognitizerId]) {
    ...
    config.state.selectedCognitizers = config.state.selectedCognitizers.filter(
      (id) => id !== newCognitizerId
    );
    saveSettings(config);
...
function showAndInitializeCognitizer(newCognitizerId) {
  if (!multiCognition) {
    // In single cognition mode, deactivate all other cognitizers
    Object.keys(activeCognitizers).forEach((key) => {
      if (key !== newCognitizerId) {
        activeCognitizers[key] = false;
        // Hide the webview if it exists
  ...
  if (!selectedWebview || loadedCognitizers[currentCognitizerId] === false) {
    if (!selectedWebview) {
      ...
      // Just append for now, we'll reorder everything when updating layout
      document.getElementById('cognitizer-container').appendChild(selectedWebview);
    }
    // Add safety check for baseUrls
    if (config.baseUrls && config.baseUrls[currentCognitizerId]) {
      selectedWebview.src = config.baseUrls[currentCognitizerId]['url'];
      loadedCognitizers[currentCognitizerId] = true;
    } else {
      console.error(`BaseURL not found for cognitizer: ${currentCognitizerId}`);
      console.error('Available baseUrls:', Object.keys(config.baseUrls || {}));
      // Fallback to a default URL
      selectedWebview.src = 'https://www.google.com/';
      loadedCognitizers[currentCognitizerId] = true;
    }
  }
  ...
  if (multiCognition) {
    ...
    // Even in single cognition mode, ensure webviews are in correct order
    reorderWebviewsBasedOnSidebar();
  }
...
function updateCognitizersLayout() {
  ...
  // First, reorder all webviews based on sidebar order
  reorderWebviewsBasedOnSidebar();
...
// Function to ensure webviews are always in the same order as sidebar
function reorderWebviewsBasedOnSidebar() {
  const container = document.getElementById('cognitizer-container');
  const cognitizerButtons = document.querySelectorAll('.cognitizer-dropdown-section > div');
  const existingWebviews = Array.from(container.querySelectorAll('webview'));
  
  // Create an array of webview IDs in the order they should appear
  const desiredOrder = [];
  cognitizerButtons.forEach((button) => {
    const cognitizerId = button.id.replace('button', '');
    if (document.getElementById(cognitizerId)) {
      desiredOrder.push(cognitizerId);
    }
  });
  
  // Check if reordering is actually needed
  const currentOrder = existingWebviews.map(w => w.id);
  const needsReorder = !desiredOrder.every((id, index) => {
    return !currentOrder[index] || currentOrder[index] === id;
  });
  ...
  // Only move webviews that are out of position
  let referenceNode = null;
  desiredOrder.forEach((cognitizerId) => {
    const webview = document.getElementById(cognitizerId);
    if (webview && webview.parentNode === container) {
      if (referenceNode && referenceNode.nextSibling !== webview) {
        // Only move if it's not already in the right position
        container.insertBefore(webview, referenceNode.nextSibling);
      } else if (!referenceNode && container.firstChild !== webview) {
        // Should be first but isn't
        container.insertBefore(webview, container.firstChild);
      }
      referenceNode = webview;
    }
  });
}

function selectBrowserURL(url) {
...
function navigateHomeCognitizer() {
  ...
  newWebview.src = config.baseUrls[cognitizerId]['url'];
...
// Selects the next cognitizer (cognitizerId + 1) that is selected, if multicognition is disabled. Otherwise do nothing
function selectNextCognitizer() {
  if (!multiCognition) {
    let newCognitizerId = parseInt(currentCognitizerId.replace('custom', '')) + 1;

    if (newCognitizerId > Object.keys(config.baseUrls).length) {
      newCognitizerId = 1;
    }
    newCognitizerId = `custom${newCognitizerId}`;

    selectCognitizer(newCognitizerId);
  }
}

// Selects the previous cognitizer (cognitizerId - 1) that is selected, if multicognition is disabled. Otherwise do nothing
function selectPreviousCognitizer() {
  if (!multiCognition) {
    let newCognitizerId = parseInt(currentCognitizerId.replace('custom', '')) - 1;

    if (newCognitizerId <= 0) {
      newCognitizerId = Object.keys(config.baseUrls).length;
    }
    newCognitizerId = `custom${newCognitizerId}`;
...
function toggleCognitizerView(only_show = false, only_hide = false) {
  ...
  if (cognitizerViewActive) {
    ...
    if (multiCognition) {
      if (
        Object.values(activeCognitizers).filter(Boolean).length === 0 ||
        Object.values(activeCognitizers).includes(false)
      ) {
...
function focusWebview(cognitizerId) {
  if (!cognitizerId) return;

  let activeWebview = document.getElementById(cognitizerId);
  if (!activeWebview) return;

  try {
    // Focus the webview element first
    activeWebview.focus();

    // Simple function to attempt focusing the textarea
    const focusTextarea = () => {
      activeWebview
        .executeJavaScript(
          `
      ...
      `
        ...
        .catch(() => {
          // Silently ignore executeJavaScript errors
        });
    };

    // If the webview is ready, focus immediately
    if (activeWebview.isReady) {
      focusTextarea();
    } else {
      // Otherwise wait for dom-ready
      const domReadyHandler = () => {
        activeWebview.isReady = true;
        focusTextarea();
        activeWebview.removeEventListener('dom-ready', domReadyHandler);
      };
      activeWebview.addEventListener('dom-ready', domReadyHandler);
    }
  } catch (error) {
    console.error('Error in focusWebview:', error);
  }
}
...
function loadSettings() {
  try {
    ...
    if (os.platform() === 'darwin') {
      ...
      const userDataPath = path.join(os.homedir(), '.instant-cognition');
    ...
    if (!loadedConfig.state.selectedCognitizers) {
      // Check if we have the old singular format
      if (loadedConfig.state.selectedCognitizer) {
        loadedConfig.state.selectedCognitizers = [loadedConfig.state.selectedCognitizer];
      } else {
    ...
    if (Array.isArray(loadedConfig.baseUrls)) {
      console.log('Converting baseUrls array to object format');
      let newBaseUrls = {};
      loadedConfig.baseUrls.forEach((item, index) => {
        // Handle both string URLs and object format
        if (typeof item === 'string') {
          // Legacy format - just a URL string
          newBaseUrls[`custom${index + 1}`] = {
    ...
    // Apply settings that don't require DOM elements
    if (loadedConfig.settings.openLinkExternal) {
      document.getElementById('open-link-external').classList.add('selected');
    ...
    // Save settings to ensure consistent format
...
function saveSettings(config) {
  // Keep baseUrls as an object - don't convert to array
  // This ensures the structure remains consistent with name and url properties
...
window.setCollapsedMenuIcons = setCollapsedMenuIcons;
window.fullNames = fullNames;
...
function applySettings(settings) {
  ...
  if (settings.launchAtStartup) {
    ipcRenderer
      .invoke('call-isLaunchAtStartupEnabled')
      .then((isEnabled) => {
        if (!isEnabled) {
...
function writeSettingsToFile(configDataStr) {
  ...
  // Use platform-specific paths (same as loadSettings)
  if (os.platform() === 'darwin') {
    // macOS: ~/.instant-cognition/config.json
    const userDataPath = path.join(os.homedir(), '.instant-cognition');
    configPath = path.join(userDataPath, 'config.json');
...
function navigateHomeActive() {
  let activeWebview = getActiveWebview();
  let cognitizerId = activeWebview.id;
  activeWebview.src = config.baseUrls[cognitizerId]['url'];
...
function toggleMultiCognition() {
  ...
  if (multiCognition) {
    ...
    // If we have a saved state of multiple cognitizers, restore it.
    if (config.state.lastSelectedCognitizers && config.state.lastSelectedCognitizers.length > 0) {
      config.state.selectedCognitizers = [...config.state.lastSelectedCognitizers];
      config.state.lastSelectedCognitizers = []; // Clear after restoring

      // Deactivate all cognitizers before activating the saved ones
      Object.keys(activeCognitizers).forEach((key) => {
        activeCognitizers[key] = false;
        const webview = document.getElementById(key);
        if (webview) {
      ...
      // Activate the restored cognitizers
      config.state.selectedCognitizers.forEach(cognitizerId => {
        selectCognitizer(cognitizerId, true, true);
      });
    }
    updateCognitizersLayout();
    ...
    // If there are multiple cognitizers active, save their state.
    if (config.state.selectedCognitizers.length > 1) {
      config.state.lastSelectedCognitizers = [...config.state.selectedCognitizers];
    } else {
      config.state.lastSelectedCognitizers = [];
    }

    // When disabling multi-cognition:
    // 1. Keep track of which cognitizer was last active
    const lastActive = config.state.lastSelectedCognitizers.length > 0 ? config.state.lastSelectedCognitizers[0] : currentCognitizerId;

    // 2. Reset all active cognitizers except the last active one
    Object.keys(activeCognitizers).forEach((key) => {
      if (key !== lastActive) {
        activeCognitizers[key] = false;
        const webview = document.getElementById(key);
    ...
    // 4. Select and show the last active cognitizer. This will also reset selectedCognitizers to just this one.
    selectCognitizer(lastActive, false, true);
  ...
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      mutation.addedNodes.forEach((node) => {
        if (
          node.classList?.contains('cognitizer-browser') ||
...
function getActiveWebview() {
  ...
  return (
    Array.from(cognitizerWebviews).find((webview) => webview.classList.contains('active')) || null
  );
}

// Create closeActiveWebview() which will close either the browser or the specific cognitizer that is active
function closeActiveWebview() {
  const activeWebview = getActiveWebview();
  if (activeWebview) {
    if (activeWebview.classList.contains('web-browser')) {
      toggleBrowserView(false, true);
      navigateHomeBrowser();
    } else {
      closeActiveCognitizer();
    }
  }
}
...
// Must be done before DOMContentLoaded so onclick handlers work
window.selectCognitizer = selectCognitizer;
window.toggleMultiCognition = toggleMultiCognition;
window.toggleOpenLinkExternal = toggleOpenLinkExternal;
window.toggleFindBar = toggleFindBar;
window.toggleCognitizerView = toggleCognitizerView;
window.toggleBrowserView = toggleBrowserView;
window.navigateBackCognitizer = navigateBackCognitizer;
window.navigateForwardCognitizer = navigateForwardCognitizer;
window.refreshPageCognitizer = refreshPageCognitizer;
window.navigateHomeReset = navigateHomeReset;
window.navigateBackBrowser = navigateBackBrowser;
window.navigateForwardBrowser = navigateForwardBrowser;
window.refreshPageBrowser = refreshPageBrowser;
window.navigateHomeBrowser = navigateHomeBrowser;
...
function removeExcessCognitizerButtons(baseUrlsLength) {
  // Remove all buttons beyond the actual count (up to 50 as defined in HTML)
  for (let i = 50; i > baseUrlsLength; i--) {
...
// Store the current shortcut listener for cleanup
...
function registerKeyboardShortcuts() {
  ...
  // Global document keydown listener for all shortcuts
  const shortcutListener = (event) => {
    ...
    // Skip if settings menu is recording a shortcut
    if (document.body.classList.contains('recording-shortcut')) {
      return;
    }
    
    // Convert the keyboard event to a shortcut string
    const shortcutKey = shortcutManager.eventToShortcut(event);
    if (!shortcutKey) return;
    
    // Find which action this shortcut maps to
    const shortcuts = shortcutManager.getAllShortcuts();
    const matchingShortcut = shortcuts.find(shortcut => shortcut.key === shortcutKey);
  ...
  // Store the listener for cleanup and add it to the document
  currentShortcutListener = shortcutListener;
  document.addEventListener('keydown', shortcutListener);
}

// Re-register shortcuts (for when settings change)
function reregisterKeyboardShortcuts() {
  registerKeyboardShortcuts();
}

// Expose the reregister function globally for settings menu
window.reregisterKeyboardShortcuts = reregisterKeyboardShortcuts;

// Execute the action for a specific shortcut
function executeShortcutAction(action) {
  try {
    switch (action) {
      ...
      case 'cognitizer.previous':
        ipcRenderer.send('select-previous-cognitizer');
        break;
      case 'cognitizer.next':
        ipcRenderer.send('select-next-cognitizer');
        break;
      case 'cognitizer.close':
        // Send close message to active cognitizer
      ...
      // View shortcuts
      case 'view.toggle':
        // Toggle between browser and cognitizer view - use existing toggle functionality
        ipcRenderer.send('show-toggle-bar');
        break;
      case 'view.multicognition':
        ipcRenderer.send('toggleMultiCognition');
        break;
      case 'view.browser':
        ...
        break;
      case 'view.cognitizer':
        ipcRenderer.send('toggleCognitizerView');
        break;
      ...
      case 'find.toggle':
        ...
        break;
      case 'find.next':
        // Find next functionality - use existing find system
        const findNext = document.querySelector('#find-next');
        if (findNext) findNext.click();
        break;
      case 'find.previous':
        // Find previous functionality - use existing find system
        const findPrev = document.querySelector('#find-previous');
        if (findPrev) findPrev.click();
        break;
        
      // Zoom shortcuts
      case 'zoom.in':
        ipcRenderer.send('zoomInActive');
        break;
      case 'zoom.out':
        ipcRenderer.send('zoomOutActive');
        break;
      case 'zoom.reset':
        ipcRenderer.send('zoomResetActive');
        break;
        
      // Numbered cognitizer shortcuts
      default:
        if (action.startsWith('cognitizer.select.')) {
          const cognitizerNumber = parseInt(action.split('.')[2]);
          ipcRenderer.send('switch-cognitizer', cognitizerNumber);
        }
        break;
    }
  } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  findInputElement.addEventListener('click', (e) => {
    e.target.select();
  });
  ...
  if (config.state.selectedCognitizers && config.state.selectedCognitizers.length > 0) {
    ...
    config.state.selectedCognitizers.forEach((cognizerId, index) => {
      ...
      selectCognitizer(cognizerId, true, true);
    });
...
ipcRenderer.on('select-next-cognitizer', (event) => {
  selectNextCognitizer();
});

ipcRenderer.on('select-previous-cognitizer', (event) => {
  selectPreviousCognitizer();
});
...
ipcRenderer.on('closeActiveCognitizer', (event, message) => {
  closeActiveCognitizer();
});
...
ipcRenderer.on('closeActiveWebview', () => {
  closeActiveWebview();
});
...
ipcRenderer.on('selectCognitizer', (event, cognitizerId) => {
  // Make sure you have a function to switch cognitizers:
  if (typeof selectCognitizer === 'function') {
    selectCognitizer(cognitizerId);
  } else {
    console.error('selectCognitizer function not defined.');
  }
});

// Capture any CmdOrCtrl + clicked link
...
function navigateCognitizerToUrl(cognitizerId) {
  // Show input dialog using IPC
  ipcRenderer.send('show-url-prompt', cognitizerId);
}

// Add IPC listener for URL navigation response
ipcRenderer.on('navigate-to-url', (event, { url }) => {
  // Get the webview for this cognitizer
  let activeWebview = getActiveWebview();
...
// Functions already made global before DOMContentLoaded
window.navigateHomeActive = navigateHomeActive;
window.openConfig = function () {
  ipcRenderer.send('open-config-file');
};

// Also make the new components available globally
window.shortcutManager = shortcutManager;
window.sidebar = sidebar;
window.settingsMenu = settingsMenu;

// Make saveSettings available globally for components
window.saveSettings = saveSettings;
